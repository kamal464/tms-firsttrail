...
function matchPasswordValidator(control: FormControl): { [key: string]: boolean } | null {
...
info = 'noview'
  ...
  constructor(private formBuilder: FormBuilder) {
    ...
    this.companyDetails = this.formBuilder.group({
      ...
      mobile: ['', [Validators.required,patternValidator(/^(\+\d{1,3}[- ]?)?\d{10}$/)]],
      ...
      education: [''],
      marks: ['',[ patternValidator(/^(?:100(?:\.0{1,2})?|\d{1,2}(?:\.\d{1,2})?)$/)]],
      ...
      this.companyDetails.get('mobile').valueChanges
        .pipe(
 ...
 onSave() {
  ...
  onKeyDown(event: KeyboardEvent) {
    ...
    if (inputField.value.length >= 10 && event.key !== 'Backspace' && event.key !== 'Delete')  {
  ...
  oncharDown(event: KeyboardEvent){
    const inputField = event.target as HTMLInputElement;
    if(inputField.value.length >= 300 && event.key !== 'Backspace' && event.key !== 'Delete' && event.key !=='ctrl' && event.key !== 'a' ) {
      event.preventDefault();
    }
  }
  onmarksDown(event: KeyboardEvent){
    const inputField = event.target as HTMLInputElement;
    if(inputField.value.length >= 6 && event.key !== 'Backspace' && event.key !== 'Delete' ) {
      event.preventDefault();
    }
  }
