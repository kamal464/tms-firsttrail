...
export class SchemaColumnComponent implements OnInit {
  ...
  selectColumn(column: string) {
    console.log(column);
    this.SelectedColumnData = this.filteredData.find(
      (item) => item.columnname === column || item.id === column
    );
    this.columnname = this.filteredData.some(
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      console.log(params)
      const { table,fieldid} = params;
  this.title = table;
      this.fieldId = fieldid
      this.getTableSchemaColumns(table);
    });
    this.getSchemaColumn();
  }


  getTableSchemaColumns = (table: string) => {
    this.http
      .post(
      ...
      .subscribe((data) => {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          const firstRow = this.newcolumns[0];
          if (firstRow) {
  ...
  getSchemaColumn() {
    this.http
    .post(`${API_BASE_URL}/t/schemacolumn/getall`, {})
    .subscribe((data) => {
      if (data != undefined && data != null) {
        ...
        console.log(field,'dddd');
        this.filteredData = this.SchemaColumn.filter(
          (item) => item.fkschematableid == field
          );
  ...
  editColumns() {
    ...
    this.http
      .post(`${API_BASE_URL}/t/schemacolumn/update`, requestBody)
      .subscribe(() => {
